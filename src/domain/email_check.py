...
def syntax_problem(email: str) -> str | None:
    ...
    # «-botirergashov258@gmail.com»: дефис прилип от тире в строке «Автор —
    # адрес». По RFC такое имя ящика допустимо, и LOCAL_RE его пропускал, но ни
    # gmail, ни mail.ru адресов, начинающихся или кончающихся не буквой и не
    # цифрой, не заводят — письмо гарантированно отскакивает.
    if not local[0].isalnum() or not local[-1].isalnum():
        return "имя ящика начинается или кончается знаком — вероятно мусор от вёрстки"
