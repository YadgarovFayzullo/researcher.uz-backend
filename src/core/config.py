...
class Settings(BaseSettings):
    ...
    # --- Онлайн-сессии конференций (meet.researcher.uz, см. src/domain/meet.py) ---
    # Адрес форка Orange Meets на Cloudflare. Пусто = кнопка «Подключиться»
    # отвечает 503, расписание при этом работает.
    MEET_BASE_URL: str | None = None
    # Общий секрет с воркером комнаты: им подписан токен входа. Отдельный от
    # SECRET_KEY сознательно — воркер живёт у Cloudflare, и утечка его секрета
    # не должна открывать сессии самого API.
    MEET_JWT_SECRET: str | None = None
    # Сколько живёт ссылка входа. Сессии идут по 3–4 часа, плюс запас на
    # переподключение; дольше суток держать бессмысленно.
    MEET_TOKEN_EXPIRE_HOURS: int = 8
    # За сколько минут до начала участникам открывается вход (модератору — всегда).
    MEET_JOIN_BEFORE_MINUTES: int = 30
